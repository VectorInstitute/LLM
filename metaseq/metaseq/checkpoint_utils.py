    ...
    if cfg.restore_file == default_restore_file:
        ...
        if cfg.finetune_from_model is not None and first_launch:
            ...
            if PathManager.exists(cfg.finetune_from_model):
                ...
                else: # check for renamed sharded version
                    suffix_nonshard = '-model_part-'+re.findall(r'\d+', suffix)[0]
                    sharded_path = cfg.finetune_from_model.replace(".pt", suffix_nonshard + ".pt")
                    if PathManager.exists(sharded_path):
                        checkpoint_path_to_load = sharded_path
                        suffix = suffix_nonshard

            logger.info(
                f"test pretrained model from {sharded_path}, suffix {suffix}"
            )
...
def load_checkpoint(cfg: CheckpointConfig, trainer, ignore_dataloader=False, **passthrough_args):
    """
    Load a checkpoint and restore the training iterator.
    *passthrough_args* will be passed through to
    ``trainer.get_train_iterator``.
    ...
    suffix = trainer.checkpoint_suffix
    default_restore_file = "checkpoint_last.pt"
    checkpoint_path_to_load = find_checkpoint_path_to_load(cfg, suffix, default_restore_file)
    ...
    logger.info(f"loaded extra state: {extra_state is not None}")
    if extra_state is not None:
        logger.info(f"loaded extra state: {[k for k in extra_state]}")
        logger.info(f"loaded extra state: previous time: {extra_state['previous_training_time']} val_loss: {extra_state['val_loss']}")
...
def load_checkpoint_to_cpu(path, arg_overrides=None, load_on_all_ranks=False) -> dict:
    ...
    #logger.info(f"Rank {torch.distributed.get_rank()} loading {local_path}")
    print(f"Rank {torch.distributed.get_rank()}: {path}")
    try:
...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            logger.info("load checkpoint to cpu")
            if state is None:
