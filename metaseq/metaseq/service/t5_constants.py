...
CHECKPOINT_FOLDER = "/checkpoint/opt_test/original/t5-large"

###

# tokenizer files
SENTENCEPIECE_VOCAB = "/checkpoint/opt_test/original/t5-large"
# MODEL_FILE = os.path.join(CHECKPOINT_FOLDER, "reshard.pt")

# MEGATRON stuff
MODEL_FILE = os.path.join(CHECKPOINT_FOLDER, "t5reshard.pt")


LAUNCH_ARGS = [
    f"--arch transformer_t5_large",
    f"--model-parallel-size {MODEL_PARALLEL}",
    f"--distributed-world-size {TOTAL_WORLD_SIZE}",
    "--task translation",
    "--bpe hf_byte_bpe",
    f"--vocab-filename {SENTENCEPIECE_VOCAB}",  # TODO(susanz): hack for getting interactive_hosted working on public repo
    f"--path {CHECKPOINT_FOLDER}/t5reshard.pt",
    "--beam 1 --nbest 1",
