...
def get_hf_logits(client, hf_model, prompts):
    ...
    input_ids = torch.as_tensor(
        [i + [1] * (max_len - len(i)) for i in input_ids_list]
    ).cuda()
    with torch.no_grad():
...
def init_opt_hf_mappings(num_layers):
    ...
    hf_mappings = {
        "logits": ["custom", get_hf_logits, lambda output: [output]],
...
def assert_activations_correctness(
    hf_results, opt_results, act_type="transformer_layer", crash_on_false=True
):
    """
    ...
    for layer_idx, ((module_name, opt_acts), hf_acts) in enumerate(
        zip(opt_results.items(), hf_results)
    ):
        for batch_idx, (opt_act, hf_act) in enumerate(zip(opt_acts, hf_acts)):
            ...
            if not allclose:
                allclose_fails.append(
                    f"{act_type} | layer {layer_idx} | batch {batch_idx}"
                )
...
def main(args):
    ...
    for (opt_type, opt_map), (hf_type, hf_map) in zip(
        ...
        hf_acts = retrieve_hf_activations(
            hf_map, client, hf_model, prompts, aux=(batch_size,)
        )

        diff[opt_type], allclose_fails = assert_activations_correctness(
            hf_acts, opt_acts, act_type=opt_type, crash_on_false=False
        )
