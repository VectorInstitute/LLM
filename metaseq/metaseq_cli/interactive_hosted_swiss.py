...
def batching_loop(timeout=100, max_tokens=MAX_BATCH_TOKENS):
    ...
    while True:
        try:
            ...
            logger.info(f"Current longest: {longest}")
            logger.info(f"Current batch cost: {batch_cost}")
            ...
            if batch:
                ...
                if torch.distributed.is_initialized():
                    dist_utils.broadcast_object(
                        request_object,
                        src_rank=0,
                        group=dist_utils.get_global_group(),
                    )
                try:
                    generations = generator.generate(**request_object)
                except RuntimeError:
                    # CUDA probable died. Need to hard crash to kick in self-
                    # healing mechanisms
                    raise
    ...
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        ...
        while True:
            try:
                ...
                # Continue looping for the next generation
                pass
...
def completions(engine=None):
    # check for valid API key first (TODO: disabled for testing)
...
def cli_main_swiss():
    ...
    dist_utils.call_main(cfg, swiss_worker_main, namespace_args=args)
...
def cli_main():
    ...
    global port, MODE, cfg
    parser = options.get_generation_parser()
    #parser = options.get_swiss_parser()
